...
class _AddQuestionState extends State<AddQuestion> {
  ...
  Future<void> onPressedButton() async {
    ...
    //1- se disciplina errado = funciona
    //2- se conteudo errado = funciona
    //3- se conteudo e disciplina cetos = funciona
    //4- se conteudo e disciplina certos e não houver questões = não funciona (nulo)
    ...
    if (_formQuestionKey.currentState!.validate()) {
      ...
      List<Content> listaContent = []; //e se aqui for nulo?
      ...
      int id_new = 0;
        ...
        if(listaContent.isNotEmpty){
          print('ta entrando');
          listaQuestion = await QuestionDao().getLastId(listaContent[0].id);
          print('ta saindo');
          if(listaQuestion[0].id!=null){
            print(listaQuestion[0].id);
            id_new = listaQuestion[0].id+1;
            print('passou na incrementação');
          }else{
            id_new = 1;
            print('entrou na execção');
          }
        }
